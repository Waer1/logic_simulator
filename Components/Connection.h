...
public:
	...
	Connection(const GraphicsInfo& r_GfxInfo, OutputPin* pSrcPin, InputPin* pDstPin, Component* S, Component* D, int dp);

	virtual void Operate() ;	//Calculates the output according to the inputs
	virtual void Draw(Output* pOut, bool selected = false, bool IsDeleted = false);	//for each component to Draw itself

	void setSourcePin(OutputPin *pSrcPin);
	void setDestPin(InputPin *pDstPin);
	OutputPin* getSourcePin();
	InputPin* getDestPin();
	OutputPin * Getm_OutputPin();
	InputPin * Getm_InputPin(int i);
	int Getm_Inputs();
